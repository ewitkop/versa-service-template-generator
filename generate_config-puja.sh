...
W_ZoneName=$(grep "tvi-0/${TRANSPORT_VR_TVI}" "$CONFIG_FILE"  \
        | grep zone  \
        | awk '{print $12}' 
        )
...
echo -e "\e[35m  TVI ZONE : \e[1;35m$W_ZoneName\e[0m"
echo -e
...
#        if [[ -n "$TRANSPORT_VR" ]]; then
            ...
            echo "delete devices template ${DEVICE_TEMPLATE} config orgs org-services ${PROVIDER_TENANT} objects zones zone ${W_ZoneName} interface-list tvi-0/$TRANSPORT_VR_TVI.0"
#        fi
        ...
        echo "set devices template $TEMPLATE config orgs org-services ${TENANT_NAME} objects zones zone W-ST-$ROUTING_INSTANCE-${COMMON_VR} interface-list tvi-0/$TRANSPORT_VR_TVI.0"
        ...
        echo "set devices template $TEMPLATE config routing-instances routing-instance ${COMMON_VR} protocols bgp bgp ${COMMON_VR_BGP_ID} group ${TRANSPORT_VR_BGP_GROUP_NAME} neighbor $RI_TVI_IP peer-as 64514 local-address $TRANSPORT_VR_TVI_IP local-as 64515 "
