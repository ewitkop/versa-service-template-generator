...
echo -e
        ...
        echo "set devices template $DEVICE_TEMPLATE config routing-instances routing-instances Infantry-Common-VR protocols bgp bgp ${COMMON_VR_BGP_ID} routing-peer-policy Export-Only-Default term T1-Default from ip nlri prefix-list Default-Rou
te-Prefix"
        echo "set devices template $DEVICE_TEMPLATE config routing-instances routing-instances Infantry-Common-VR protocols bgp bgp ${COMMON_VR_BGP_ID} routing-peer-policy Export-Only-Default term T1-Default then accept"
        echo "set devices template $DEVICE_TEMPLATE config routing-instances routing-instances Infantry-Common-VR protocols bgp bgp ${COMMON_VR_BGP_ID} routing-peer-policy Export-Only-Default term T1-Default then rib-bgp-ecmp false"
        echo "set devices template $DEVICE_TEMPLATE config routing-instances routing-instances Infantry-Common-VR protocols bgp bgp ${COMMON_VR_BGP_ID} routing-peer-policy Export-Only-Default term T2-Reject-All then reject"
        echo "set devices template $DEVICE_TEMPLATE config routing-instances routing-instances Infantry-Common-VR protocols bgp bgp ${COMMON_VR_BGP_ID} routing-peer-policy Export-Only-Default term T2-Reject-All then rib-bgp-ecmp false"
